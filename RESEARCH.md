# ReliefKaki — product standards & comparable research
...
ReliefKaki should feel like a **high-trust caregiver activation layer**, rather than a generic gig marketplace or a care directory. The winning interaction is: **one small bounded ask → a suitable, trained person → an accountable completion record**.
...
4. **Separate help from treatment.** ReliefKaki supports navigation, logistics, technology and companionship. It does not provide medical advice, personal care or emergency response.
...
Related caregiver, discharge and social-impact winners repeatedly demonstrate one complete transaction instead of a broad feature list: a specific ask becomes an accepted action, a staff dashboard records it, and a completion or escalation follows. ReliefKaki’s judge path therefore remains **ask → scope check → offer → admin confirmation → completion receipt**, plus a visible unsafe-request redirect.
...
Caregiver-support products such as [Give InKind](https://www.giveinkind.com/features) and [Lotsa Helping Hands](https://lotsahelpinghands.com/how-it-works) turn help into specific time-bound activities with instructions, claim status and reminders. This validates the ReliefKaki task-first model.
...
ReliefKaki should hard-filter before ranking:
